...
export const AuthPage = ({ onLogin, onRegister, loading }) => {
  ...
  const [auth, setAuth] = useState({ username: "", password: "", companyName: "" });
  ...
  const handleSubmit = (e) => {
    ...
    if (mode === "login") {
      ...
      onRegister(auth.username, auth.password, auth.companyName);
    }
  };
      ...
      <div className="w-full max-w-[480px] neo-card p-12 shadow-2xl shadow-blue-500/5 animate-in zoom-in-95 duration-500">
        <div className="flex flex-col items-center mb-10">
          ...
          <h1 className="text-3xl font-black text-slate-800 tracking-tight">
            {mode === "login" ? "Welcome to Neo" : "Join Neo Admin"}
          </h1>
          <p className="text-slate-400 font-bold text-sm mt-2 text-center">
            {mode === "login" 
              ? "Sign in to manage your kiosk network" 
              : "Set up your centralized kiosk dashboard"
            }
          </p>
        </div>
          ...
          {mode === "register" && (
            <div className="space-y-2 animate-in slide-in-from-top-4 duration-300">
              ...
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Enter company / terminal name"
                  className="premium-input pl-12 h-[54px]"
                  value={auth.companyName}
                  onChange={(e) => setAuth({ ...auth, companyName: e.target.value })}
                  required
                />
              </div>
            </div>
            ...
            {mode === "login" ? (
              <p className="text-sm font-bold text-slate-500">
                ...
                <button
                  ...
                  onClick={() => {
                    setMode("register");
                    setAuth({ username: "", password: "", companyName: "" });
                  }}
                  className="text-[#1b6ef3] hover:underline cursor-pointer font-black"
                >
